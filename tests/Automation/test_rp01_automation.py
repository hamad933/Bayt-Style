...
def base(action="list_sessions"):
    return {
        "schema_version": "rp01.automation.request/v1", "request_id": "req-001", "project_id": "RP01",
        "controller_id": "CENTRAL", "logical_task_id": "task-001", "action": action,
        "repository": "hamad933/Bayt-Style",
    }


class SchemaTests(unittest.TestCase):
    def test_valid_read(self): self.assertEqual(validate_request(base()).action, "list_sessions")
    def test_unknown_key_fails_closed(self):
        req = base(); req["surprise"] = True
        with self.assertRaises(RequestValidationError): validate_request(req)
    def test_wrong_repository_fails_closed(self):
        req = base(); req["repository"] = "other/repo"
        with self.assertRaises(RequestValidationError): validate_request(req)
    def test_create_requires_exact_sha_plan_approval_and_authority(self):
        req = base("create_session")
        req.update({"write_domain":"automation-test","starting_branch":"work/example","expected_sha":"a"*40,"prompt":"bounded test","authority_ref":"drive:authority-ref","require_plan_approval":True})
    ...
    def test_changed_replay_changes_intent(self):
        one = base(); two = dict(one); two["logical_task_id"] = "task-002"
...
class RedactionTests(unittest.TestCase):
    def test_secret_fields_and_google_keys_are_redacted(self):
        ...
        self.assertEqual(safe["JULES_API_KEY"], "[REDACTED]"); self.assertNotIn("AIza", safe["message"])
