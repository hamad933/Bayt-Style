...
class S04S05Test extends TestCase
{
    ...
    public function test_variant_option_authority_is_normalized_relational_data(): void
    {
        ...
        $sand = VariantAttributeOption::where('variant_attribute_id', $color->id)
            ->where('value_ar', 'رملي')
            ->firstOrFail();
        $variant = Variant::where('sku', 'BAS-CHAIR-SAND-01')
        ...
        $this->assertSame([
            'color' => 'رملي',
            'finish' => 'جوزي داكن',
        ], $variant->optionSelection());
        $this->assertTrue($variant->attributeOptions->contains($sand));
        $this->assertDatabaseHas('variant_attribute_option_variant', [
            'variant_id' => $variant->id,
            'variant_attribute_option_id' => $sand->id,
        ]);
    }
    ...
    public function test_multi_sellable_variant_featured_product_requires_configuration_on_home(): void
    {
        ...
        $this->get('/')
            ->assertOk()
            ->assertSee('data-testid="configure-variants-'.$chair->id.'"', false)
            ->assertSee('اختر الخيارات')
            ->assertDontSee('data-testid="quick-add-'.$chair->id.'"', false);
    }
    ...
    public function test_single_sellable_variant_featured_product_retains_exact_quick_add(): void
    {
        ...
        $this->get('/')
            ->assertOk()
            ->assertSee('data-testid="quick-add-'.$lamp->id.'"', false)
