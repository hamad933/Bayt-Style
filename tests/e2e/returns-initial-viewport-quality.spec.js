...
async function createOrderAndOpenReturns(page) {
    ...
    await page.getByLabel('البريد الإلكتروني').fill('returns-viewport@example.test');
    await page.getByLabel('رقم الجوال').fill('+966500000018');
    ...
    await page.getByLabel('الشارع / سطر العنوان').fill('شارع اختبار المرتجعات 18');
    await page.getByLabel('المبنى / الوحدة اختياري').fill('وحدة 18');
...
for (const viewport of [
    ...
    test(`S08 returns truth is visible in the initial viewport ${viewport.name}`, async ({ page }) => {
        ...
        await expectInsideInitialViewport(page.getByRole('heading', { name: 'المرتجعات والاسترداد', exact: true }), viewport);
        const eligibility = page.getByTestId('return-eligibility-state');
        await expectInsideInitialViewport(eligibility, viewport);
        await expect(eligibility).toContainText('طلب المرتجع غير متاح حاليًا');
        await expect(eligibility).toContainText('لن نفترض التسليم أو الأهلية');
        ...
        await page.screenshot({
            path: `storage/test-artifacts/visual/s08-returns-initial-${viewport.name}.png`,
            fullPage: false,
        });
    });
}
